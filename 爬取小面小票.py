...
# YM_mongo配置
YM_client = pymongo.MongoClient(host='106.14.94.38', port=3717, username='root', password='vCV3HexFslp')
YM_mongo_db = YM_client.xiaomian_prod_report
...
def getOrderTicketIdsByTimeFromMongo(db_name, begin_query_tag, start, end):
    ...
    query = {
        ...
        "payload.body.hex_discount_list.thirdcode": {"$exists": True},
    ...
    mongoret = db_name.pos.find(query, {"store_name": 1, "payload.body.order_no": 1, "sales_date": 1,
                                        ...
                                        # "payload.body.hex_discount_list.thirdcode": 1
                                        })
    ...
    for r in mongoret:
        ...
        if len(discounts) > 0:
            for n in range(len(discounts)):
                ...
                third_codes = discounts[n].get('thirdcode') or []
...
def main():
    ...
    mkpath = "/Users/hws/Desktop/" + for_filename + "/"
    # 调用函数
    mkdir(mkpath)
    ...
    ticket_info_list = getOrderTicketIdsByTimeFromMongo(YM_mongo_db, "sales_date", '{}T16:00:00'.format(start_date),
                                                        '{}T16:00:00'.format(end_date))

    # filename = 'DQ全门店特定电子小票信息{}'.format(for_filename)
    filename = 'YM全门店特定电子小票信息{}'.format(for_filename)
